...
function match!(
  ...
  inverse_covariance_matrix = samplecovar(dat[!, t], covariate_data; variancesonly = variancesonly)
...
function eligibility!(
  ...
  if isnothing(exposure)
    # Standard matching without exposure considerations
    ...
    # Apply crossover window constraints to determine match eligibility
    eligiblematches!(
      observations, matches,
      time_groups, treatment_groups, ids, fmin, fmax, treatcat
    )
  else
    # Exposure-based matching with additional exposure history constraints
    ...
    eligiblematches!(
      observations, matches,
      time_groups, treatment_groups, ids, fmin, fmax, treatcat, exg = exposure_groups
    )
  end
...
function eligibility!(
  ...
  # Create efficient indexed views of data organized by (treatment_time, unit_id)
  # Handles missing values in covariate matrix
  if isnothing(exposure)
    # Standard matching without exposure considerations
    covariate_groups, time_groups, treatment_groups = make_groupindices(
      dat_t, dat_trt,
    ...
    eligiblematches!(
      observations, matches,
      covariate_groups, time_groups, treatment_groups, ids, fmin, fmax, treatcat
    )
  else
    # Exposure-based matching with additional exposure history constraints
    covariate_groups, time_groups, treatment_groups, exposure_groups = make_groupindices(
    ...
    eligiblematches!(
      observations, matches,
      covariate_groups, time_groups, treatment_groups, ids, fmin, fmax, treatcat, exg = exposure_groups
    )
  end
...
function eligibility!(
  ...
  observation_count = length(dat_t)
